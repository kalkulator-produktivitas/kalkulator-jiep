    ...
    <main class="h-[92vh] w-[100vw]">
      ...
      <div v-if="mode == 'kpi'" class="py-4">
        <div class="mb-4 flex gap-8 items-center">
          <select class="select select-bordered" :value="selectedValue.tahun" @change="onKpiYearSelected">
            <option
        ...
        <div class="py-4 grid grid-cols-3 gap-8 h-[65vh]">
          <div class="flex flex-col gap-2 overflow-y-auto">
            <button type="button" v-for="indicator in selectedValue.indicators"
              :class="`px-6 py-4 rounded-lg border border-neutral-200 hover:border-blue-500 hover:bg-blue-50 text-left transition ease-in-out flex justify-between ${selectedIndicator?.id === indicator.id ? 'border-blue-500 bg-blue-50' : ''}`"
              @click="selectedIndicator = indicator">
              <p class="ml-2 w-full text-left">{{ indicator.name }}</p>
              <p class="font-bold">{{ formatNumber(indicator.value) }}</p>
            </button>
          </div>
            ...
            <table class="table text-sm">
              ...
              <tbody class="text-xs">
                <tr
                  ...
                  :class="`hover:bg-blue-50 transition ease-in-out cursor-pointer text-sm ${kpi.id === selectedKpi?.id ? 'bg-blue-50' : ''}`"
